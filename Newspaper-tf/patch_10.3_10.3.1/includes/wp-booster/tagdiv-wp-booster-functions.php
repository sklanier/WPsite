	...
	function tagdiv_check_theme_version() {
        ...
        if ( false !== $response ) {
            ...
            usort( $zip_resources, function( $val_1, $val_2) {
                ...
                return version_compare($val_1, $val_2 );
            });

            $zip_resources = array_reverse( $zip_resources );

            foreach ( $zip_resources as $index => $zip_resource ) {
                if ( ! empty( $zip_resource['title']['rendered'] ) && ! empty( $zip_resource['source_url'] ) && strpos( $zip_resource['title']['rendered'], TD_THEME_NAME ) >= 0 ) {
